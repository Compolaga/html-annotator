...
# (`class="band"`) of een titel met "to-do". Die twee samen zijn nodig: `class="num"`
...
while IFS= read -r f; do
  grep -q '<span class="num">' "$f" 2>/dev/null || continue
...
if [ "$UITLEG" -eq 1 ]; then
  ...
  echo "punten: $(grep -c '<span class="num">' "$BESTE")"
  echo "hoogste nummer: $(grep -oE '<span class="num">[0-9]+' "$BESTE" | grep -oE '[0-9]+' | sort -n | tail -1)"
