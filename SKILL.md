...
Verificatie: `tests/run.sh`, uitleg in `tests/criteria.md`, volledige freeze in
...
Wat Luc kan (gedrag ongewijzigd; details in het handbook):
...
Bij `.` zelf de open ronde zoeken (`toon-annotaties.py --open` of de bridge).
Eerst begrijpen, dan doorvoeren, dan `POST /resolve`. Lees
`references/agent-handbook.md` (deel 4) voor refs, locators, hunks, crops.
`toon-annotaties.py` print de werkregel opnieuw als er iets openstaat.
